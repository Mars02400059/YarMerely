                     ...
                     isIncoming:(BOOL)isIncoming
{
    ...
    if (self) {
        ...
        _timeLabel.text = @"";
        _timeLength = 0;
        _chatter = session.sessionChatter;//配置跟谁聊天
        
        [[EaseMob sharedInstance].callManager removeDelegate:self];
        [[EaseMob sharedInstance].callManager addDelegate:self delegateQueue:nil];//成为代理.
...
//    [self showHint:NSLocalizedString(@"call.rejected", @"Reject the call")];
    UIAlertView * a = [[UIAlertView alloc] initWithTitle:@"" message:NSLocalizedString(@"call.rejected", @"Reject the call") delegate:nil cancelButtonTitle:@"好" otherButtonTitles:nil, nil];
    [a show];
    a = nil;
...
//    [self showHint:NSLocalizedString(@"call.init", @"Is init the call...")];
    [self _stopRing];
    UIAlertView * a = [[UIAlertView alloc] initWithTitle:@"" message:NSLocalizedString(@"call.init", @"Is init the call...") delegate:nil cancelButtonTitle:@"好" otherButtonTitles:nil, nil];
    [a show];
    a = nil;
    AVAudioSession *audioSession = [AVAudioSession sharedInstance];
...
//    [self showHint:NSLocalizedString(@"call.dealloc", @"Is hanging up the call...")];
    UIAlertView * a = [[UIAlertView alloc] initWithTitle:@"" message:NSLocalizedString(@"call.dealloc", @"Is hanging up the call...") delegate:nil cancelButtonTitle:@"好" otherButtonTitles:nil, nil];
    [a show];
    a = nil;
    AVAudioSession *audioSession = [AVAudioSession sharedInstance];
